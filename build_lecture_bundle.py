...
DETAILED_LECTURE_BULLETS: dict[str, list[str]] = {
    "S1-Q02": [
        "课堂起点：老师先把这题放在江苏归纳概括训练里看。江苏这类题常要求“一段话”作答，不能写成一二三四的条列；阅卷时一段话和语言流畅都有分值，所以答案要先想好总领句、分类角度和句间衔接。",
        "审题第一步：题干真正要概括的是“显著成效”。“智慧执法方面的创新举措”是直接修饰语，不能完全不写，但只需高度概括为智慧执法新举措、行政行为码、智慧交通运输管理系统等；主要篇幅要放在这些做法带来的结果。",
        "审题第二步：做材料时先分清做法和成效。系统如何开发、由哪些功能构成、如何运行，大多属于做法；材料中出现“让、推动、提高、有效、从源头上、提供支撑”等结果性表达，才是要进入答案的成效。",
        "材料一：行政行为码本身、赋码规则、覆盖范围、掌上执法端操作、专题培训和推广应用都是做法。真正要写的是当事人可查询、执法人员可回溯、全流程信息清楚；后文又给出更规范的表述，即推动执法权力全流程在线运行、及时预警、精准监督、风险可控、留痕可溯。把这些合并成“让执法阳光高效”。",
        "材料二：智慧交通运输管理系统的研发、源头数据采集、不停车称重、预警提示等是做法。成效要抓四层：提升货运车辆超载超限执法能力，提升管理效能，从源头上消除超载超限，节约检查时间并增加驾驶员收入。材料二的总分类可归为“提升执法能力”。",
        "材料三：人群聚集热力图、5G 视频巡逻、防走失系统是三个智慧防控做法。对应成效分别是为安保指挥决策提供科学依据、弥补固定监控缺陷、帮助找寻走失儿童和走散游客；开头“未发生重大刑事治安案件、未发生踩踏事故、社会治安秩序良好”可作为这一材料的总效果，归为“维护社会治安秩序”。",
        "材料四：AI 小秘书的企业信息协查、报告助手、政策法规问答都是功能做法。成效分别是提高日常执法效率、保证执法记录规范性和准确性、为环境违法案件精准查处提供可靠支撑；分类句可概括为“助力移动执法”。",
        "归并方法：四则材料最好不要按“行政行为码、系统、热力图、AI 小秘书”机械罗列，而要用“在什么方面”的分类角度串起来：一是让执法阳光高效，二是提升执法能力，三是维护社会治安秩序，四是助力移动执法。这样既保留创新举措，又突出成效。",
        "答案组织：总领句写“智慧执法新举措成效显著”，后面用一段话顺次展开四个方面。每个方面先给分类短语，再接该材料中最规范、最能概括成效的原文或近原文表达，避免把系统构成、操作流程和背景问题写成答案主体。",
        "易错提醒：第一，不能只写“行政行为码”等做法，忽略成效；第二，不能见到材料前面的铺垫场景就照抄；第三，江苏题一般不需要背所谓规范词，材料里往往已经给出可用表述，关键是把零散表达合并成同一层级的采分点。",
    ],
}
